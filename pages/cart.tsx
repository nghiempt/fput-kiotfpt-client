...
const Page = () => {
  ...
  const handleSelect = (e: any, product: any, section: any) => {
    ...
    if (checkSelected(product)) {
      setTotalPrice(totalPrice - product?.total);
      const index = seletecItems.findIndex(
  ...
  const remove = async (e: any, item: any) => {
    ...
    if (res?.result) {
      try {
        const [car] = await Promise.all([CartService.getCart()]);
        if (car?.result) {
          setCart(car?.data);
        }
      } catch (error) {
  ...
  const submit = () => {
    if (checkUpdateProfile(profile)) {
      ...
      localStorage.setItem("cart", JSON.stringify(seletecItems));     
  ...
  const init = async () => {
    ...
    try {
      const [car, pros] = await Promise.all([
        CartService.getCart(),
                            ...
                            {section?.items?.map((item: any, ind: any) => {
                              return (
                                <div
                                  key={ind}
                                  onClick={(e) =>
                                    handleSelect(e, item, section)
                                  }
                                  className={`${
                                    checkSelected(item)
                                      ? "border-gray-700 border-2"
                                      : "border-gray-200"
                                  } w-full cursor-pointer rounded-lg border bg-white p-4 shadow-sm md:p-6`}
                                >
              ...
              <div className="mx-auto mt-6 max-w-4xl flex-1 space-y-6 lg:mt-0 lg:w-full">
                <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
                  ...
                  <div className="flex items-center justify-center gap-2">
                    <button
                      onClick={submit}
                      className="inline-flex mt-10 items-center gap-2 text-[16px] font-medium text-primary-700 hover:font-black"
                    >
                      {checkUpdateProfile(profile)
                        ? "Checkout"
                        : "Update profile to checkout"}
                      <svg
                        className="h-5 w-5"
                        aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg"
